    ...
    >
      ...
      <div className={classes.quotedMsg}>
        ...
        {message.quotedMsg.mediaType === "audio"
          && (
            <div className={classes.downloadMedia}>
              <AudioModal url={message.quotedMsg.mediaUrl} message={message.quotedMsg} />
...
const renderStatusReplyPreview = (message) => {
  if (!message.statusReply || !message.statusId) {
    return null;
  }

  console.log("🖼️ Rendering status preview for message:", message);

  // Parse the status content if it exists
  ...
  try {
    if (message.statusReply) {
      console.log("📝 Parsing statusReply:", message.statusReply);
      // Try to extract media information from statusReply
      const statusData = JSON.parse(message.statusReply);
      if (statusData) {
        // Extract content from various possible locations
        statusContent = statusData.body || statusData.text || statusData.caption || "";
        
        // Extract media information - check multiple possible structures
        if (statusData.mediaUrl) {
          statusMediaUrl = statusData.mediaUrl;
          statusMediaType = statusData.mediaType;
        } else if (statusData.message) {
          // Sometimes the data is nested in a 'message' property
          const messageData = statusData.message;
          if (messageData.imageMessage) {
            statusMediaType = "image";
            statusMediaUrl = messageData.imageMessage.url || messageData.imageMessage.directPath;
            statusContent = statusContent || messageData.imageMessage.caption || "";
          } else if (messageData.videoMessage) {
            statusMediaType = "video";
            statusMediaUrl = messageData.videoMessage.url || messageData.videoMessage.directPath;
            statusContent = statusContent || messageData.videoMessage.caption || "";
          } else if (messageData.audioMessage) {
            statusMediaType = "audio";
            statusMediaUrl = messageData.audioMessage.url || messageData.audioMessage.directPath;
          } else if (messageData.documentMessage) {
            statusMediaType = "document";
            statusMediaUrl = messageData.documentMessage.url || messageData.documentMessage.directPath;
            statusContent = statusContent || messageData.documentMessage.fileName || "";
          }
        }
        
        console.log("✅ Parsed status data:", { statusContent, statusMediaUrl, statusMediaType });
      }
    }
  } catch (e) {
    console.log("⚠️ Failed to parse statusReply as JSON, treating as text:", e);
    // If parsing fails, treat statusReply as simple text
    statusContent = message.statusReply;
  }
  ...
  return (
    <div
      ...
      style={{ marginBottom: "8px", backgroundColor: "#e8f5e8" }}
    >
      <span
        className={clsx(classes.quotedSideColorLeft, {
          [classes.quotedSideColorRight]: false, // Status is always from others
        })}
        style={{ backgroundColor: "#25D366" }} // WhatsApp green for status
      ></span>
      <div className={classes.quotedMsg}>
        <span className={classes.messageContactName} style={{ color: "#25D366" }}>
          📸 Status Original
        </span>
        
        {/* Render status media if available */}
        {statusMediaUrl && statusMediaType === "image" && (
          <div style={{ marginTop: "4px", marginBottom: "4px" }}>
            <ModalImageCors 
              imageUrl={statusMediaUrl.startsWith('http') 
                ? statusMediaUrl 
                : `${process.env.REACT_APP_BACKEND_URL}/public/company${user.companyId}/${statusMediaUrl}`
              } 
            />
          </div>
        )}
        
        {statusMediaUrl && statusMediaType === "video" && (
          <div style={{ marginTop: "4px", marginBottom: "4px" }}>
            <video 
              src={statusMediaUrl.startsWith('http') 
                ? statusMediaUrl 
                : `${process.env.REACT_APP_BACKEND_URL}/public/company${user.companyId}/${statusMediaUrl}`
              }
              style={{ 
              ...
              muted
              onError={(e) => {
                console.log("❌ Failed to load status video:", statusMediaUrl);
        ...
        {statusMediaUrl && statusMediaType === "audio" && (
          <div style={{ marginTop: "4px", marginBottom: "4px" }}>
            <AudioModal 
              url={statusMediaUrl.startsWith('http') 
                ? statusMediaUrl 
                : `${process.env.REACT_APP_BACKEND_URL}/public/company${user.companyId}/${statusMediaUrl}`
              } 
            />
        ...
        {statusMediaUrl && statusMediaType === "document" && (
          <div style={{ marginTop: "4px", marginBottom: "4px" }}>
            <Button
              startIcon={<GetApp />}
              variant="outlined"
              size="small"
              target="_blank"
              href={statusMediaUrl.startsWith('http') 
                ? statusMediaUrl 
                : `${process.env.REACT_APP_BACKEND_URL}/public/company${user.companyId}/${statusMediaUrl}`
              }
            >
        ...
        {/* Render status text content */}
        {statusContent && (
          <div style={{ fontSize: "13px", color: "#666", marginTop: "4px" }}>
            {statusContent.length > 100 ? `${statusContent.substring(0, 100)}...` : statusContent}
          </div>
        )}

        {/* If no content available, show generic message */}
        {!statusContent && !statusMediaUrl && (
          ...
          <React.Fragment key={message.id}>
            ...
            >
              ...
              {message.statusReply && (
                <div>
                  <span className={classes.forwardMessage}
                  ><RadioButtonChecked style={{ color: "#25D366", fontSize: "16px" }} /> {i18n.t("messagesList.statusReply")}
                  </span>
                  <br />
                </div>
              )}
              ...
              <div className={clsx(classes.textContentItem, {
                ...
                {message.statusReply && renderStatusReplyPreview(message)}
